...
def test(model_dir, data_dir, results_subdir, random_seed, resolution):
    ...
    output_dir  = os.path.join(results_subdir, "classification_results_res_"+np.str(2**log2_record)+"/test")
    ...
    if use_best_weights:
        ...
        model_weights_path = os.path.join(results_subdir, "classification_results_res_"+np.str(2**log2_record)+"/train/best_weights.h5")
    else:
        print("** Using LAST weights",flush=True)
        model_weights_path = os.path.join(results_subdir, "classification_results_res_"+np.str(2**log2_record)+"/train/weights.h5")
