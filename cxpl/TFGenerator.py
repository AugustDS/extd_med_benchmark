...
class TFWrapper(Sequence):
    ...
    def get_all_test_data(self):
        ...
        for i in range(0,self.steps):
            x,y = K.get_session().run(batch)
            assert x.shape == (self.batch_size, self.model_target_size[0], self.model_target_size[1],3)
            assert y.shape == (self.batch_size, self.label_size)
            if i == 0:
                all_x = x
                all_y = y
            else:
